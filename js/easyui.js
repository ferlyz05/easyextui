(function ($) {
    $.parser = {
        ...
        plugins: ["draggable", "droppable", "resizable", "pagination", "tooltip", "linkbutton", "menu", "menubutton", "splitbutton", "progressbar", "tree", "combobox", "combotree", "combogrid", "numberbox", "validatebox", "searchbox", "numberspinner", "timespinner", "calendar", "datebox", "datetimebox", "slider", "layout", "panel", "datagrid", "propertygrid", "treegrid", "tabs", "accordion", "window", "dialog", "buttongroup", "toolbar"],
        parse: function (_2) {
            var aa = [];
...
(function ($) {
    ...
    function _1dd(_1de, _1df) {
        ...
        if (opts.frame) {
            _1e0.addClass("panel-frame");
            _1e1.addClass("panel-header-frame");
            _1e2.addClass("panel-body-frame");
        };
        if (_1df) {
            $.extend(opts, {
                width: _1df.width,
    ...
    function _208(_209, _20a) {
        ...
        if (_20a == true) {
            ...
            _20b.removeClass("panel-collapse");
        } else {
            _20b.removeClass("panel-collapse");
            body.show();
            opts.collapsed = false;
    ...
    $.fn.panel.parseOptions = function (_228) {
        var t = $(_228);
        return $.extend({}, $.parser.parseOptions(_228, ["id", "width", "height", "left", "top", "title", "iconCls", "cls", "headerCls", "bodyCls", "tools", "href", "frame", {
            cache: "boolean",
            fit: "boolean",
            border: "boolean",
            noheader: "boolean"
    ...
    function _271(_272, _273, _274) {
        ...
        if (_274) {
            ...
            for (var _275 in _274) {
                $("<a></a>").attr("href", "javascript:void(0)").text(_275).css("margin-left", 6).bind("click", eval(_274[_275])).appendTo(tb).linkbutton();
            }
        }
        win.window({
            title: _272,
...
(function ($) {
    function _284(_285) {
        ...
        if (!isNaN(opts.height)) {
            ...
            _289 = cc.outerHeight() - (_288 + 5) * _28a.length;
        } else {
            cc.css("height", "");
        }
...
(function ($) {
    ...
    function _375(_376, _377, _378) {
        ...
        function _37e() {
            ...
            if (_377 == "east") {
                var ww = _380.width + _37a.width - 44;
                ...
                if (_377 == "west") {
                    ...
                    if (_377 == "north") {
                        ...
                        if (!_35a(_379.expandNorth)) {
                            hh += _37a.height - 44 + ((_37a.split || !_37a.border) ? 1 : 0);
                        }
...
(function($) {
    $('input:not([type="radio"]):not([type="checkbox"]), select, textarea, fieldset').each(function() {
        var elem = $(this);
        var padding = parseInt(elem.parent().css("padding-right"));
        var width = elem.parent().outerWidth() - ((padding * 3));
        elem.width(width);
        elem.css("background-position-x", (width - 20));
    });
    $('input[type="radio"], input[type="checkbox"]').each(function() {
        var elem = $(this);
        var type = elem.attr("type");
        var title = elem.attr("title") || "";
        var wrap = elem.wrap($('<label class="'+type+'">')).parent();
        $('<span>').addClass(type+"-btn").appendTo(wrap);
        $('<span>').text(title).appendTo(wrap);
    });
})(jQuery);
(function($) {
    var Buttongroup = {
        init: function($elem) {
            var self = this;
            this.opts = $elem.buttongroup('defaults');
            $elem.panel(this.opts);
        }
    };
    $.fn.buttongroup = function(options, defaults) {
        if (options && typeof options === 'string') {
            return $.fn.buttongroup.methods[options](this, defaults);
        };
        return this.each(function() {
            var $elem = $(this),
                buttongroup = Object.create(Buttongroup),
                data = $.data(this, 'buttongroup'),
                opts;
            if (data) {
                opts = $.extend(data.defaults, options);
            } else {
                opts = $.extend({}, $.fn.buttongroup.defaults, options, $.fn.buttongroup.parseOptions(this));
                data = $.data(this, 'buttongroup', {
                    defaults: opts,
                    buttongroup: $elem,
                });
            }
            buttongroup.init($elem);
        });
    };
    $.fn.buttongroup.methods = {
        defaults: function($elem) {
            return $elem.data('buttongroup').defaults;
        },
        buttongroup: function($elem) {
            return $elem.data('buttongroup').buttongroup;
        }
    };
    $.fn.buttongroup.parseOptions = function (options) {
        return $.extend({}, $.parser.parseOptions(options, ["title"]));
    };
    $.fn.buttongroup.defaults = $.extend({}, $.fn.panel.defaults, {
        title: 'Button Group',
        border: false,
        cls: 'buttongroup',
        headerCls: 'buttongroup-header',
        bodyCls: 'buttongroup-body',
    });
})(jQuery);
(function($) {
    var Toolbar = {
        init: function($elem) {
            var self = this;
            var opts = $elem.toolbar('defaults');
            var wrap = $elem.parent().addClass('toolbar');
            var tbar = $elem.addClass('toolbar-'+opts.position);
            var height = tbar.outerHeight();
            var width = tbar.outerWidth();
            if (opts.position == "top") {
                wrap.css('padding-top', height + parseInt(wrap.css('padding-top')));
            };
            if (opts.position == "bottom") {
                wrap.css('padding-bottom', height + parseInt(wrap.css('padding-bottom')));
            };
            if (opts.position == "left") {
                wrap.css('padding-left', width + parseInt(wrap.css('padding-left')));
            };
            if (opts.position == "right") {
                wrap.css('padding-right', width + parseInt(wrap.css('padding-right')));
            };
            wrap.bind('scroll', function(e) {
                var top = $(this).scrollTop();
                if (opts.position == "bottom") {
                    top = top += (wrap.outerHeight() - height);
                };
                tbar.css('top', top);
            });
        }
    };
    $.fn.toolbar = function(options, defaults) {
        if (options && typeof options === 'string') {
            return $.fn.toolbar.methods[options](this, defaults);
        };
        return this.each(function() {
            var $elem = $(this),
                toolbar = Object.create(Toolbar),
                data = $.data(this, 'toolbar'),
                opts;
            if (data) {
                opts = $.extend(data.defaults, options);
            } else {
                opts = $.extend({}, $.fn.toolbar.defaults, options, $.fn.toolbar.parseOptions(this));
                data = $.data(this, 'toolbar', {
                    defaults: opts,
                    toolbar: $elem,
                });
    ...
    $.fn.toolbar.parseOptions = function(options) {
        return $.extend({}, $.parser.parseOptions(options, ["position"]));
    }
    $.fn.toolbar.methods = {
        defaults: function($elem) {
            return $elem.data('toolbar').defaults;
        },
        toolbar: function($elem) {
            return $elem.data('toolbar').toolbar;
        }
    };
    $.fn.toolbar.defaults = {
        position: "top"
